...
const Badminton = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://backend-ecommer-dq0g.onrender.com/products/badminton")
    ...
    return (
        <>
            ...
            <h4 className='headings'>BADMINTON</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 331 && item.id <= 345 }).map((element, index) => (
