...
const BeautyToolsAndAccessories = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://backend-ecommer-dq0g.onrender.com/products/beautytoolsandaccessories")
    ...
    return (
        <>
            ...
            <h4 className='headings'>BEAUTY TOOLS AND ACCESSORIES</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 256 && item.id <= 270 }).map((element, index) => (
...
export default BeautyToolsAndAccessories