...
const DietAndNutrition = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://backend-ecommer-dq0g.onrender.com/products/dietandnutrition")
    ...
    return (
        <>
            ...
            <h4 className='headings'>DIET AND NUTRITION</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 271 && item.id <= 285 }).map((element, index) => (
