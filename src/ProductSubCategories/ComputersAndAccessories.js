...
const ComputersAndAccessories = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://backend-ecommer-dq0g.onrender.com/products/computersandaccessories")
    ...
    return (
        <>
            ...
            <h4 className='headings'>COMPUTERS AND ACCESSORIES</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 31 && item.id <= 45 }).map((element, index) => (
