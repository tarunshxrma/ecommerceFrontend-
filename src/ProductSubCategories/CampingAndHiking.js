...
const CampingAndHiking = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://backend-ecommer-dq0g.onrender.com/products/campingandhiking")
    ...
    return (
        <>
            ...
            <h4 className='headings'>CAMPING AND HIKING</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 376 && item.id <= 390 }).map((element, index) => (
...
export default CampingAndHiking