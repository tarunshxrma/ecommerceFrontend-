...
const FictionBooks = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://backend-ecommer-dq0g.onrender.com/products/fictionbooks")
    ...
    return (
        <>
            ...
            <h4 className='headings'>FICTION BOOKS</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 421 && item.id <= 435 }).map((element, index) => (
