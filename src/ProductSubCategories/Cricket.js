...
const Cricket = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://backend-ecommer-dq0g.onrender.com/products/cricket")
    ...
    return (
        <>
            ...
            <h4 className='headings'>CRICKET</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 316 && item.id <= 330 }).map((element, index) => (
